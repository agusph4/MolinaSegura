...
<?php if(!empty(get_post_meta($id,'meta-image', true))){?>
	...
	<?php if(!empty(get_post_meta($id,'pdfp_onei_pp_pgname', true))):?>Nombre del Archivo :  <?php $url =get_post_meta($id,'meta-image', true);$name = basename($url); echo $name; ?><br/> <?php endif;?>
  ...
  $viewer_base_url= plugins_url()."/pdf-poster/pdfjs/web/viewer.php"; $final_url = $viewer_base_url."?file=".$file_name."&download=".$pdf_download."&print=".$print."&openfile=".$openfile;
	...
	?>
	<!--<a href="<?php /*echo $final_url; */?>"><button class="btn btn-secondary">Pantalla Completa</button></a>--><br /><iframe loading="lazy" width="<?php echo $pdf_width;?>" height="<?php echo $pdf_height;?>" src="<?php echo $final_url;?>"></iframe>
