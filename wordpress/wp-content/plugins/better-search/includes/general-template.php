...
function get_bsearch_header( $search_query, $numrows, $limit ) {
	...
	$output .= sprintf( __( 'Resultados <strong>%1$s</strong> - <strong>%2$s</strong> de <strong>%3$s</strong>', 'better-search' ), $first, $last, $numrows );

	$output .= '
	  </td>
	  <td width="50%" style="text-align:right">';
	/* translators: 1: Current page number, 2: Total pages */
	$output .= sprintf( __( 'Página <strong>%1$s</strong> de <strong>%2$s</strong>', 'better-search' ), $current, $total );
	...
	$output .= __( 'Resultados por página', 'better-search' );
...
function get_bsearch_footer( $search_query, $numrows, $limit ) {
	...
	if ( 0 != $page ) { //phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
		...
		$output   .= __( 'Anterior', 'better-search' );
		$output   .= "</a>    \n";
	}
	...
	if ( ! ( ( ( $page + $limit ) / $limit ) >= $pages ) && 1 != $pages ) { //phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
		...
		$output   .= __( 'Siguiente', 'better-search' );
		$output   .= ' &raquo;</a>';
	}
	$output .= '</p>';
